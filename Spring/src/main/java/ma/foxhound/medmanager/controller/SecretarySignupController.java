...
@AllArgsConstructor
public class SecretarySignupController {
    ...
    public ResponseEntity<?> signup(@RequestBody LoginRequestDto request) {
        ...
        secretary.setHashedPassword(passwordEncoder.encode(request.getPassword()));
        userRepository.save(secretary);

        return ResponseEntity.status(201).body(java.util.Map.of("message", "secretary created", "username", request.getUsername()));
    }
